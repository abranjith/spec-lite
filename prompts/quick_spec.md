...
**This is NOT a replacement for the full Planner → Feature pipeline.** Use Quick Spec when:
...
> **Scope Check**: If at any point the requirements expand beyond what fits a single feature spec (you're looking at 15+ tasks, multiple independent features, or broad architectural decisions), pause and suggest: "This is growing beyond quick-spec territory. I'd recommend switching to the Planner to create a proper multi-feature plan. Want me to do that instead?"
...
- **Assign a FEAT-ID**: Use `FEAT-QS-001` format (QS = Quick Spec) to distinguish from plan-derived features. If the user has a specific ID preference, use theirs.
...
3. **Format**: `- [ ] <description> (discovered during: FEAT-QS-<ID>)`
...
```markdown
<!-- Generated by spec-lite v0.0.6 | sub-agent: quick_spec | date: {{date}} -->
...
**Quick Spec**: "Got it — email notifications on order shipment. A few questions before I spec this out:
...
**Quick Spec**: "Simple and clear. Let me confirm a few details:
...
This feels well-scoped for a quick spec. I'll confirm my approach and then break it into tasks."
